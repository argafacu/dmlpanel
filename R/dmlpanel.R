...
#' @param C1   A p x p1 matrix. Default is NULL.
#' @param C2   A q x p2 matrix. Default is NULL.
...
dmlpanel <- function(y,v,w,id,C1=NULL,C2=NULL, Omega=NULL, S2=NULL, TotT,  L = 5, re=1.1, indreg){
  ...
  if(!is.null(C2)){
    ...
    for(l in 1:length(I)){
      ...
      for(j1 in IDs_not_fold){
        ...
        q2 <- diag(TotT) - v2 %*% ginv(t(v2) %*% v2) %*% t(v2)
        h2 <- ginv(v2)
    ...
    for(l in 1:length(I)){
      ...
      cn <- (re/(sqrt(n_cl*TotT)))*qnorm(1-gamma/(2*ncol(w))) #Regularization parameter from Belloni et al. (2016)
  ...
  if(is.null(S2)){
    ...
    if(!is.null(Omega)){
      ...
      for(uh in 1:ncol(v)){
        ...
        var <- var(gsmestlr) - 2*mean(gfmestlrv)*cov(gsmestlr, gfmestlrv) - 2*mean(gfmestlrv)*(cov(gsmestlr, gfmestlrv) - 2*mean(gfmestlrv)*var(gfmestlrv))
      ...
      for (uh in 1:ncol(pairs)){
        ...
        gsmestlr <- Re(unlist(gsmestlr))
        gfmestlrv1 <- Re(unlist(gfmestlrv1))
        gfmestlrv2 <- Re(unlist(gfmestlrv2))
        ...
        var <- var(gsmestlr) + (mean(gfmestlrv2)^2)*var(gfmestlrv1) + (mean(gfmestlrv1)^2)*var(gfmestlrv2) - 2*mean(gfmestlrv2)*cov(gsmestlr,gfmestlrv1) -2*mean(gfmestlrv1)*cov(gsmestlr,gfmestlrv2) + 2*mean(gfmestlrv1)*mean(gfmestlrv2)*cov(gfmestlrv1,gfmestlrv2)

        seest <- sqrt(var)/sqrt(n)


        Resultscov[[paste0("v", i, "v", j)]] <- list("Est." = Re(est), "S.e." = Re(seest))
    ...
    if(!is.null(Omega)){
      ...
      for(l in 1:length(I)){
        ...
        for (j1 in IDs_not_fold) {
          ...
          part1 <- S2%*%ginv((diag(TotT)%x%q2)%*%S2)
      ...
      for (uh in 1:ncol(pairs)){
        ...
        Resultscov[[paste0("v", i, "v", j)]] <- list("Est." = Re(est), "S.e." = Re(sees))
  ...
  if(!is.null(C1)){
    ...
    QC1 <- diag(ncol(w)) - C1%*%ginv(t(C1)%*%C1)%*%t(C1)
  ...
  return(list("FM" = Re(Resultsfm), "SM"=Re(Resultssm), "Cov"= Resultscov, "CP"=Re(Resultsbeta)))
